...
data_manager = Agent(
    name="data_manager",
    model=Gemini(model="gemini-2.0-flash", retry_options=retry_config),
    description="Управляет данными пользователя: сохранение еды, получение истории, цели.",
    instruction="""Ты менеджер данных для системы отслеживания питания.
...
root_agent = Agent(
    ...
    model=Gemini(model="gemini-2.0-flash", retry_options=retry_config),
    description="""
    AI-помощник для отслеживания питания. 
    Анализирует еду, считает калории, дает рекомендации, 
    отвечает на вопросы об истории питания.
    """,
